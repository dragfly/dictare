...
from voxtype.stt.faster_whisper import (
    _MODEL_REPOS,
...
class TestGetCachedModelPath:
    ...
    @patch("voxtype.stt.faster_whisper.os.path.exists", return_value=True)
    @patch("voxtype.stt.faster_whisper.os.path.dirname", return_value="/cache/models--Systran/snapshots/abc123")
    ...
    def test_turbo_delegates_to_turbo_function(self) -> None:
        ...
        with patch("voxtype.stt.faster_whisper._get_turbo_model_path", return_value="/turbo/path") as mock:
    ...
    def test_unknown_model_delegates_to_turbo(self) -> None:
        ...
        with patch("voxtype.stt.faster_whisper._get_turbo_model_path", return_value=None):
...
class TestGetTurboModelPath:
    ...
    @patch("voxtype.stt.faster_whisper.os.path.exists", return_value=True)
    @patch("voxtype.stt.faster_whisper.os.path.dirname", return_value="/cache/turbo")
...
class TestIsCached:
    ...
    def test_is_model_cached_true(self) -> None:
        with patch("voxtype.stt.faster_whisper._get_cached_model_path", return_value="/some/path"):
            assert _is_model_cached("base") is True

    def test_is_model_cached_false(self) -> None:
        with patch("voxtype.stt.faster_whisper._get_cached_model_path", return_value=None):
            assert _is_model_cached("base") is False

    def test_is_turbo_cached_true(self) -> None:
        with patch("voxtype.stt.faster_whisper._get_turbo_model_path", return_value="/turbo"):
