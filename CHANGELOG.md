...
## [3.0.0-alpha.115] - 2026-02-12

### Changed

- **Migrate client-side code to OpenVIP SDK**: all HTTP calls to engine now use `openvip.Client` instead of raw `urllib`.
- `_read_from_sse()` uses `Client.subscribe(reconnect=True)` with SDK-managed backoff and reconnection.
- `_poll_active_agent()`, `_check_engine()`, `_try_start_service()` use `Client.get_status()` / `Client.is_available()`.
- `StatusPanel._fetch_status()` and `engine_status` CLI command use `Client.get_status()`.

### Fixed

- `engine status` and `service status` CLI commands now read from `platform.*` (was reading non-existent top-level keys, showing "unknown" for all fields).
...
- `PipelineLoader` with `inspect.signature()`-based DI for auto-wiring pipeline steps.
- `register_step()` / `get_step_registry()` API for step registration.
- Built-in steps registered at import time (`agent_filter`, `input_filter`, `agent_switch`, `input`).

### Changed

- `engine.py` uses `PipelineLoader` instead of hardcoded filter/executor construction.

## [3.0.0-alpha.110] - 2026-02-12

### Changed

- `_read_from_sse()` uses `InputExecutor` pipeline instead of inline `x_input` extraction.
- `InputExecutor` now handles `newline` field (appends `\n` to text).

## [3.0.0-alpha.109] - 2026-02-12

### Removed

- `voxtype.output` module (`WebhookSender`) — dead code, replaced by engine HTTP server + SSE architecture.
