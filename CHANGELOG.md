...
### Fixed
- **TTS venv missing openvip dependencies** — the worker's isolated venv had
  pydantic but not urllib3/python-dateutil/typing-extensions, causing openvip
  import to fail even when PYTHONPATH was correct. Added all openvip deps to
  shared venv packages. Better error logging (full traceback on import failure).
...
### Fixed
- **TTS worker crash logging** — worker errors (e.g., missing openvip import,
  subscribe failures) now log full details to `tts-worker.log` instead of
  being swallowed. Engine warning includes the actual error message.
- **Config template headers** — `[pipeline.submit_filter]` and
  `[pipeline.agent_filter]` headers are now always present (not commented).
  Users only uncomment values.

## [0.1.10] - 2026-02-25

### Fixed
- **TOML section parser confused by array literals** — multi-line TOML arrays
  like `["ok", "invia"]` were mistaken for section headers `[section]`,
  truncating the submit filter config in the dashboard editor. Fixed with
  regex that distinguishes `[section.name]` from `["array", "value"]`.
...
### Changed
- **Extract TTSManager from engine.py** — all TTS state (engine loading, worker
  subprocess, speech methods, mic-pausing counter, voice listing) now lives in
  `core/tts_manager.py`. Engine delegates via thin wrappers. Reduces engine.py
  from 1703 to 1329 lines (-374 lines, -22%).

## [0.1.7] - 2026-02-25

### Changed
- **Extract AgentManager from engine.py** — all agent state (registry, switching,
  output mode, grace period) now lives in `core/agent_manager.py`. Engine delegates
  via thin wrappers. Reduces engine.py by ~210 lines (1922 → 1713).
- **AgentManager.restore_session()** — proper public API for session restore,
  replacing direct private attribute mutation from engine.
...
### Fixed
- **TTS voice is now best-effort across all engines** — if the configured voice
  is unavailable, each engine logs a warning and falls back to the language
  default instead of crashing:
  - **Piper**: download failure (404, wrong name, network error) → fallback
  - **Kokoro**: `kokoro.create()` failure (unknown voice) → fallback + cache
