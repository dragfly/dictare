...
- Named session continuity (phase 2): per-agent session adapters bind each
  named session to the exact underlying agent session, so `--continue`
  resumes precisely the conversation that name refers to. Claude and Gemini
  sessions get their UUID assigned at launch (Claude sessions also carry the
  dictare name in Claude's own resume picker); Codex and Pi sessions are
  discovered from their session stores after the run (cwd-verified, safe
  with parallel sessions); Aider gets dictare-managed named history files —
  multiple named sessions per folder, which aider lacks natively. Unbound
  entries fall back to the profile's plain `continue_args`.
...
### Changed
- Stable release publishing now updates the Homebrew tap with the bootstrap
  formula instead of a Homebrew-owned Python runtime formula.

### Fixed
- Global hotkey sequences configured for Dictare are now filtered from the
  terminal input stream, so they no longer leak keystrokes to the agent.
- Output-device changes no longer reset the audio input stream; only input
  device changes trigger a reconnect.
...
### Added
- New cross-platform runtime-store installer: `curl -fsSL https://raw.githubusercontent.com/dragfly/dictare/main/install.sh | bash`.
- Dictare-owned runtime layout under `~/.local/share/dictare/versions/<version>` with stable `current` and `previous` symlinks.
- Stable user shim at `~/.local/bin/dictare`.
- `dictare upgrade`, `dictare rollback`, and `dictare repair` for product-owned lifecycle management.
- Installer support for release-candidate rollback preloading via `--previous-version`.
...
### Migration
- Existing Homebrew users should run `scripts/uninstall.sh` to remove the legacy install while preserving config, models, and user data.
- RC testers can install with `--version 0.5.0rc1 --previous-version 0.4.0` and return to the prepared stable runtime with `dictare rollback`.
...
### Fixed
- Apple Silicon installs now pin the tested MLX stack used by the stable v0.2.7 lockfile. This prevents fresh Homebrew/PyPI installs from drifting to newer `mlx`/`mlx-audio`/`mlx-lm`/transitive versions that can fail STT with `RuntimeError: There is no Stream(gpu, 1) in current thread`.
...
### Fixed
- Linux: PATH reminder at end of install now shows correctly (was checking the wrong variable)
