...
def _check_dependencies_internal() -> tuple[list, bool, list, list]:
    ...
    """
    from dictare.utils.platform import check_dependencies
        ...
        if missing_with_hints:
            ...
            for result in missing_with_hints:
                if result.install_hint and result.install_hint not in seen_hints:
                    ...
                    console.print(f"  [cyan]{hint}[/]")
        console.print("\n[dim]Or run: dictare dependencies resolve[/]")
...
def deps_resolve(
    ...
    Examples:
        dictare dependencies resolve           # Install missing deps
        dictare dependencies resolve --dry-run # Show what would be installed
    """
    import subprocess
    ...
    if all_ok:
        ...
        console.print("[yellow]Some dependencies still missing (run 'dictare dependencies check' for details)[/]")
        raise typer.Exit(1)
