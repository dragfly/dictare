...
class StatusPanel:
    ...
    def _build_panel(self) -> Panel:
        ...
        device_markup = None
        if stt_device == "cpu":
            from dictare.utils.hardware import detect_nvidia_gpu, is_mlx_available
        ...
        if not tts_available and tts_engine:
            lines.append(
                "[yellow]  Fix: dictare dependencies resolve[/]"
            )
        ...
        if self._is_loading():
            ...
            return Panel(
                content,
                title=f"dictare v{version} - Loading",
                border_style="yellow",
    ...
    def run(self) -> None:
        ...
        with Live(
            Panel("[dim]Connecting...[/]", title="dictare", border_style="yellow", width=self.PANEL_WIDTH),
