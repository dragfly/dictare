...
class NoVAD(VADEngine):
    """Dummy VAD that always returns 1.0 (no filtering)."""

    def is_speech(self, audio_chunk: NDArray[np.float32]) -> float:
        """Always returns 1.0 (no VAD filtering)."""
        return 1.0
...
class EnergyVAD(VADEngine):
    """Simple energy-based VAD for platforms without Silero support.

    Uses RMS energy with adaptive threshold to detect speech.
    Less accurate than Silero but works without ONNX dependencies.
    """

    CHUNK_SIZE = 512  # Match SileroVAD for compatibility
    ...
    def __init__(
        ...
        """
        self.threshold = threshold
        self.neg_threshold = threshold * 0.7
    ...
    def is_speech(self, audio_chunk: NDArray[np.float32]) -> float:
        ...
        if rms < self._noise_floor * 1.5:
            self._noise_floor = (
                ...
                rms * self._adaptation_rate
            )

        # Calculate speech probability based on energy above noise floor
        if rms < self._noise_floor * 2:
            return 0.0
        elif rms > self._noise_floor * 5:
            return 1.0
        else:
            # Linear interpolation between thresholds
            return (rms - self._noise_floor * 2) / (self._noise_floor * 3)
...
def get_vad_engine(
    ...
    """Get the best available VAD engine for the current platform.

    Returns SileroVAD if faster-whisper is available, otherwise EnergyVAD.
    """
    try:
        ...
        # Fallback to energy-based VAD (less accurate but no dependencies)
