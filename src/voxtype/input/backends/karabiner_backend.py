...
Karabiner runs shell commands on keypress, which communicate with voxtype
via Unix socket.
...
class KarabinerBackend(DeviceBackend):
    ...
    def _generate_config(
        ...
        for key_name, command in bindings.items():
            ...
            rule = {
                "description": f"voxtype: {key_name} -> {command}",
                "manipulators": [
                    {
                        "type": "basic",
                        "from": {
        ...
        config = {
            "title": "voxtype presenter controls",
            "rules": rules,
        }

        config_path = KARABINER_COMPLEX_MODS / "voxtype.json"
...
3. Run voxtype with your device profile - it will generate Karabiner config
